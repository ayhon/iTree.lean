...
set_option pp.universes true in #print loop
...
def ret (r : R) : iTree E R where
  curr : Unit  := ()
  step _ := .ret r

set_option pp.universes true in #print ret

def vis {α} (ev : E α) (k : α → iTree E R) : iTree E R where
  curr : Unit ⊕ (a : α) × (k a ).State := .inl ()
  step 
...
mutual 
  variable {R : Type _}{E : Type _ → Type _}
...
abbrev ObsEq{E R} (it₁ it₂ : iTree E R) := Rel (· = ·) it₁ it₂
...
def ObsEq.coinductWith
  ...
  (progress :
    ∀ (it₁ it₂ : iTree E R),
      pred it₁ it₂ → func pred it₁.unfold it₂.unfold) :
    ObsEq it₁ it₂ := by
...
instance : Std.Refl (@ObsEq E R) where refl := ObsEq.refl
instance : Std.Symm (@ObsEq E R) where symm := ObsEq.symm
instance : Trans (@ObsEq E R) (@ObsEq E R) (@ObsEq E R) where trans := ObsEq.trans _ _ _
...
def ObsEq.step_vis_obsEq_self {E R A} (it : iTree E R) (ev : E A) (kS : A → it.State) (a : A) :
  ...
  apply ObsEq.coinductWith (fun 
    (iTree.mk (State := State₁) curr₁ step₁) 
    (iTree.mk (State := State₂) curr₂ step₂) =>
    ∃ (H : State₂ = (Unit ⊕ (_ : A) × State₁)),
    curr₂ = H.mpr (Sum.inr ⟨a, curr₁⟩) ∧
    ∀ (s : State₁), 
    step₂ (H ▸ (.inr ⟨a, s⟩)) = H.symm ▸ (step₁ s).mapState (.inr ⟨a, ·⟩)
  )
  case baseCase => 
    simp [Obs.mapState]
  case progress =>
...
instance instEquiv {E R} : Equivalence (fun (x y : iTree E R) => x ≈ y) where
  refl :=  ObsEq.refl
  symm := ObsEq.symm _ _
  trans := ObsEq.trans _ _ _

instance instSetoid.{a,e,r} {E : Type a → Type e}{R : Type r}: Setoid (iTree.{a,e,r} E R) where
  r := (· ≈ ·)
  iseqv := instEquiv

def iTree'.{a,e,r} (E : Type a → Type e) (R : Type r) : Type _ := Quotient (instSetoid (E := E) (R := R))

set_option pp.universes true in #print iTree'

def iTree'.mk {E : Type _ → Type _} {R : Type _}(it : iTree E R)  := Quotient.mk inferInstance it

def iTree'.unfold (it' : iTree' E R) : Obs E R (iTree' E R) := 
  it'.lift (fun it =>
    match it.unfold with 
    | Obs.ret v => Obs.ret v
    | Obs.tau it₂ => Obs.tau (iTree'.mk it₂)
    | Obs.vis ev k => 
      Obs.vis ev (fun a => iTree'.mk (k a))
  ) (by 
    intros it₁ it₂ heq
    match h₁ : it₁.unfold, h₂ : it₂.unfold with
    | Obs.ret _, Obs.ret _ => 
...
def iTree'.out (it' : iTree' E R) : iTree E R where
    curr := it'
    step := iTree'.unfold
    
def iTree'.ret {R : Type _} (v : R){E : Type _ → Type _} := iTree'.mk (.ret v : iTree E R)

def iTree'.tau {R : Type _} {E : Type _ → Type _} (it : iTree' E R) := 
    it.lift (Quotient.mk inferInstance <| iTree.tau ·) (by 
      intros it it' heq; simp; apply Quotient.sound
      apply ObsEq.tau_congr heq
    )

def iTree'.vis {A : Type a} {R : Type _}{E : Type a → Type _} (ev : E A) (k' : A → iTree' E R) : iTree' E R := 
  let k := fun a => (k' a).out
  iTree'.mk (.vis ev k)

set_option pp.universes true in #print iTree'

def iTree'.cases.{r,a,e,s} {R : Type r}{E : Type a → Type e} {motive : iTree'.{a,e,r,s} E R → Type _} 
  (ret : (v : R) → motive (.ret v))
  (vis : ∀ {A : Type a} (ev : E A) (k : A → iTree' E R), motive (iTree'.vis ev k))
  (tau : (it : iTree' E R) → motive (.tau it)) : 
    ∀ (it' : iTree' E R), motive it' := by
  sorry
